...
import { getToken, getSession, trackScrobble, trackLove, trackUnlove, trackUpdateNowPlaying, getUserinfo } from '@main/modules/lastFM'
...
export default () => {
  ...
  mainHandle<LX.LastFM.Base & { username: string }, LX.LastFM.UserGetInfoResponse>(WIN_MAIN_RENDERER_EVENT_NAME.last_fm_get_user_info, async ({ params: data }) => {
    return getUserinfo(data.api_key, data.secret, data.username)
  })
}
