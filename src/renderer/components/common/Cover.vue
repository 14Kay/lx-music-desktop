<template>
  <div class="cover" :class="{ 'cover-hover': coverHover }" @mouseover="focus = true" @mouseleave="focus = false"
    ...
    <div class="cover-container">
      <div class="shade">
        <button v-show="focus || isPlaying" class="play-button" :style="playButtonStyles" @click.stop="$emit('play')">
          <PhPause v-if="isPlaying" size="50%" weight="fill" />
          <PhPlay v-else size="50%" weight="fill" />
        </button>
      </div>
...
const { imageUrl, fixedSize, playButtonSize, alwaysShowShadow, isPlaying } = toRefs(props)
